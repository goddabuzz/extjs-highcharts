 ...
 * @version 0.7
 * 
 * @requires HighChart 2.3.3 {@link http://www.highcharts.com}
 *
 * HighchartsAdapter
 * Updated for Highcharts release 2.3.3
 */
(function(win, Ext){
    win.HighchartsAdapter = {
        ...
        adapterRun: function(element, method){
          return parseInt(Ext.get(element).getStyle(method),10);
        },
        ...
        addEvent: function(el, event, fn) {
            ...
            if (xel) {
                ...
                if (!el.addListener) {
                    el.observer = new Ext.util.Observable();
                }
                el.observer.addListener(event, fn, el)
            }
            return el;
        },
        
      	washMouseEvent: function (e) {
      		return e.browserEvent||e;
      	},
        ...
        fireEvent: function(el, event, eventArguments, defaultFunction) {
            ...
            if (el.observer && el.observer.fireEvent) {
                el.observer.fireEvent(event, o);
        ...
        removeEvent: function(el, event, fn) {
          var observer = el.observer;
          if (observer && observer.removeListener && observer.purgeListeners) {
            if (event && fn) {
                observer.removeListener(event, fn)
            } else {
                observer.purgeListeners();
            }
          } else {
            var xel = Ext.get(el);
            if (xel) {
